...
export async function handleValidityRequest(
  ...
  if (deps.rateLimitKv) {
    ...
    if (!quota.allowed) {
      return json({ ok: false, error: { code: 'RATE_LIMITED', message: `You've reached the validity check limit. It reopens ${waitPhrase(quota.resetAt)}.`, resetAt: quota.resetAt } });
    }
  }
