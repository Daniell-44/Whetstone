...
export default function EmailCapture({ source, heading, sub, compact = false }: Props) {
  ...
  if (state === 'done') {
    return (
      <div class={compact ? 'py-2' : ''}>
        <p class="text-sm text-ink">You are on the list.</p>
      </div>
    );
  }
  ...
  if (compact) {
    return (
      <form onSubmit={submit} class="flex flex-wrap items-center gap-x-3 gap-y-2">
        <span class="font-mono text-[11px] uppercase tracking-[0.08em] text-muted">{heading}</span>
        <input
          type="email"
          autocomplete="email"
          required
          ...
          class="flex-1 min-w-[12rem] rounded-md border border-hairline bg-surface px-3 py-1.5 text-xs
        ...
        <button
          ...
          class="rounded-md bg-accent-support px-3 py-1.5 text-xs font-medium text-white
        ...
        {state === 'error' && <p class="w-full text-xs text-accent">{errMsg}</p>}
      </form>
    );
  }
  ...
  return (
    <div>
      <p class="font-mono text-[11px] uppercase tracking-[0.08em] text-muted mb-1.5">{heading}</p>
      {sub && <p class="text-sm text-muted mb-3">{sub}</p>}
      <form onSubmit={submit} class="flex flex-col sm:flex-row gap-2 max-w-md">
        <input
          ...
          placeholder="you@example.com"
          aria-label="Email address"
          class="flex-1 rounded-md border border-hairline bg-surface px-3 py-2 text-sm
                 focus:border-accent-support focus:outline-none focus:ring-1 focus:ring-accent-support
                 disabled:bg-paper"
          disabled={state === 'loading'}
        />
        <button
          type="submit"
          disabled={invalid || state === 'loading'}
          class="shrink-0 rounded-md bg-accent-support px-4 py-2 text-sm font-medium text-white
                 hover:bg-accent-support/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {state === 'loading' ? 'Sending…' : 'Notify me'}
        </button>
      </form>
      {state === 'error' && <p class="text-xs text-accent mt-2">{errMsg}</p>}
