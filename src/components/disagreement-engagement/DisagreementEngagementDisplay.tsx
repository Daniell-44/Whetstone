...
const QUALITY_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  absent:         { label: 'Absent',              cls: 'bg-hairline    text-ink'    },
};
...
const VERDICT_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  absent:   { label: 'Largely absent',      cls: 'bg-hairline    text-ink'    },
};
...
function EngagementCard({ e }: { e: OpposingPositionEngagement }) {
  ...
  return (
    <div class="rounded-lg border border-hairline bg-surface p-4">
      ...
      <p class="text-sm text-ink-strong font-medium leading-snug mb-1.5">"{e.position}"</p>
      <p class="text-xs text-ink leading-relaxed mb-2">
        <span class="font-semibold text-ink">Author's treatment:</span> {e.authorTreatment}
      </p>
      {e.triggerPassage && (
      ...
      <p class="text-xs text-ink leading-relaxed mb-2">{e.whyThisQuality}</p>
      {e.strongerVersion && (
        <div class="rounded bg-amber-50 border border-amber-200 px-3 py-2 mb-2">
          ...
          <p class="text-xs text-ink leading-relaxed">"{e.strongerVersion}"</p>
        </div>
      )}
      <p class="text-xs text-ink italic leading-relaxed">
  ...
  return (
    <div class="space-y-4">
      ...
      {result.engagements.length === 0 ? (
        <p class="text-sm text-muted">No significant opposing positions identified.</p>
      ) : (
        <div class="space-y-3">
